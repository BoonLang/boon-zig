...
const source_slots = [_]SourceSlot{
    .{ .id = 0, .semantic_id = "sources.editor.event.change" },
    .{ .id = 1, .semantic_id = "sources.editor.event.key_down" },
};

const semantic_source_map = [_]SourceMapEntry{
    .{ .semantic_id = "sources.editor.event.change", .source_path = "examples/source_physical/cells/cells.bn", .boon_start = 62, .boon_end = 68, .generated_start = 621, .generated_end = 685, .source_slot_id = 0 },
    .{ .semantic_id = "sources.editor.event.key_down", .source_path = "examples/source_physical/cells/cells.bn", .boon_start = 91, .boon_end = 97, .generated_start = 685, .generated_end = 751, .source_slot_id = 1 },
};
const render_blueprint: RenderBlueprint = .{ .node_count = 21 };
...
fn validateGeneratedRuntimePlan() !void {
    if (generated_runtime_plan.record_shapes.len != 3) return error.GeneratedRuntimePlanMismatch;
...
var generated_physical_source_slots = [_]physical_ir.SourceSlot{
    ...
    .{ .id = 1, .semantic_id = "sources.editor.event.key_down", .payload_type = "KeyEvent", .source_span = .{ .start = 91, .end = 97 }, .value_slot = 15 },
};
...
var generated_physical_state_slots = [_]physical_ir.StateSlot{
    .{ .id = 0, .semantic_id = "hold.editing_formula", .flow_node = 10 },
    .{ .id = 1, .semantic_id = "hold.commit_count", .flow_node = 20 },
};
...
var generated_physical_instructions = [_]physical_ir.Instruction{
    ...
    .{ .load_number = .{ .dst = 5, .value = 0 } },
    .{ .eval_flow_node = 6 },
    .{ .eval_flow_node = 7 },
    .{ .eval_flow_node = 8 },
    .{ .eval_flow_node = 9 },
    .{ .hold = .{ .dst = 10, .state_slot = 0, .initial = 5, .updates = &generated_physical_hold_10_updates } },
    .{ .load_number = .{ .dst = 11, .value = 0 } },
    .{ .eval_flow_node = 12 },
    .{ .eval_flow_node = 13 },
    .{ .eval_flow_node = 14 },
    .{ .eval_flow_node = 15 },
    .{ .load_state = .{ .dst = 16, .state_slot = 1 } },
    .{ .load_number = .{ .dst = 17, .value = 1 } },
    .{ .binary = .{ .dst = 18, .operator = .add, .lhs = 16, .rhs = 17 } },
    .{ .then_value = .{ .dst = 19, .source = 15, .value = 18 } },
    .{ .hold = .{ .dst = 20, .state_slot = 1, .initial = 11, .updates = &generated_physical_hold_20_updates } },
};
